...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xd:doc scope="stylesheet">
    ...
    <!-- Internal variable to check if certain extensions are available --><xsl:variable name="xslVersion">
      <xsl:choose>
         <xsl:when test="number(substring(system-property('xsl:version'), 1, 1)) &gt; 1">
   ...
   <xsl:template name="checkEXSL">
      <xsl:if test="$useEXSL = string(false()) and $xslVersion = 1">
         <xsl:message terminate="yes">XSLT Processor is not capable of using EXSL or not version
                2.0</xsl:message>
      </xsl:if>
   </xsl:template>
    <!--
