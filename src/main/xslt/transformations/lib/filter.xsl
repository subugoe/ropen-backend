...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:strip-space elements="TEI:persName TEI:placeName"/>
   <xsl:template match="/">
   ...
   <xsl:template match="TEI:addName" mode="filter"/>
   <xsl:template match="TEI:persName|TEI:placeName" mode="filter">
      <xsl:choose>
         <xsl:when test=".//exist:match">
            <xsl:element name="exist:match">
               <xsl:copy>
                  <xsl:for-each select="@*">
   ...
   <xsl:template name="filter">
      ...
      <xsl:apply-templates select="$nodes" mode="filter"/>
   </xsl:template>
...
--><xsl:template name="shorten">
      ...
      <xsl:variable name="focusNode"
                    as="element()"
                    select="$content//*[local-name(.) = $focus][1]"/>
      <xsl:variable name="width"
                    as="xs:integer"
                    select="$length - string-length($focusNode/text())"/>
      <xsl:choose>
         <xsl:when test="$width &lt; 0">
            <xsl:value-of select="$focusNode"/>
         </xsl:when>
         <xsl:otherwise>
            <xsl:call-template name="shorten-helper">
               <xsl:with-param name="context" select="$focusNode"/>
               <xsl:with-param name="width" select="$width"/>
            </xsl:call-template>
         </xsl:otherwise>
      </xsl:choose>
   </xsl:template>
   <xsl:template name="shorten-helper">
      <xsl:param name="context" as="element()"/>
      <xsl:param name="width" as="xs:integer"/>
      <xsl:variable name="precedingSiblings" select="$context/preceding-sibling::*"/>
      <xsl:variable name="followingSiblings" select="$context/following-sibling::*"/>
      <xsl:variable name="siblings"
                    select="$precedingSiblings | $context | $followingSiblings"/>
      <xsl:choose>
         <xsl:when test="$width &lt; string-length(string-join($siblings/text(), ''))">
            <xsl:variable name="sideCharCount" select="$width div 2"/>
            <xsl:variable name="cutPreceding">
               <xsl:for-each select="1 to count($precedingSiblings)">
                  <xsl:if test="string-length(string-join(subsequence($precedingSiblings, 1, .), '')) &gt;$sideCharCount">
                     <xsl:value-of select="."/>
                  </xsl:if>
               </xsl:for-each>
            </xsl:variable>
            <xsl:variable name="cutFollowing">
               <xsl:for-each select="1 to count($followingSiblings)">
                  <xsl:if test="string-length(string-join(subsequence($followingSiblings, 1, .), '')) &gt; $sideCharCount">
                     <xsl:value-of select="."/>
                  </xsl:if>
               </xsl:for-each>
            </xsl:variable>
            <xsl:variable name="cutPreceding">
               <xsl:choose>
                  <xsl:when test="not(empty($cutPreceding[1]))">
                     <xsl:value-of select="$cutPreceding[1]"/>
                  </xsl:when>
                  <xsl:otherwise>
            ...
            <xsl:variable name="cutFollowing">
               <xsl:choose>
                  <xsl:when test="not(empty($cutFollowing[1]))">
                     <xsl:value-of select="$cutFollowing[1]"/>
                  </xsl:when>
                  <xsl:otherwise>
                     <xsl:value-of select="'1'"/>
                  </xsl:otherwise>
            ...
            <xsl:value-of select="subsequence($precedingSiblings, 1, $cutPreceding) | $context | subsequence($followingSiblings, 1, $cutFollowing)"/>
         </xsl:when>
         <xsl:otherwise>
            <xsl:call-template name="shorten-helper">
               <xsl:with-param name="context" select="$context/.."/>
               <xsl:with-param name="width"
                               select="$width - string-length(string-join($siblings/text(), ''))"/>
            </xsl:call-template>
         </xsl:otherwise>
      </xsl:choose>
   </xsl:template>
