...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:function name="ropen:count-non-whitespace" as="xs:integer">
        <xsl:param name="text" as="text()*"/>
        <xsl:value-of select="count($text[not(matches(., '\s*'))])"></xsl:value-of>
    </xsl:function>
    ...
    <xsl:function name="ropen:ltrim" as="xs:string">
        ...
        <xsl:value-of select="replace($str, '^\s+', '')"></xsl:value-of>
    </xsl:function>
    ...
    <xsl:function name="ropen:trim" as="xs:string">
        <xsl:param name="str" as="xs:string"/>
        <xsl:value-of select="ropen:ltrim(ropen:rtrim($str))"></xsl:value-of>
    </xsl:function>
    ...
    <xsl:function name="ropen:uri-to-name" as="xs:string">
        ...
        <xsl:variable name="file-name" select="replace($uri, '^.*/(.*)$', '$1')" as="xs:string"></xsl:variable>
    ...
    <xsl:function name="ropen:file-exists" as="xs:boolean">
        <xsl:param name="location" as="xs:string"></xsl:param>
        <xsl:choose>
            <xsl:when test="document($location)">
                <xsl:copy-of select="true()"></xsl:copy-of>
            </xsl:when>
            <xsl:otherwise>
