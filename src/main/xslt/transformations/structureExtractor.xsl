...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                exclude-result-prefixes="xd xs METS TEI xsl"
                version="2.0">
   ...
   <xsl:variable name="output"
    ...
    -->
    <!-- input seed here --><xsl:output method="xml"
               encoding="UTF-8"
      ...
      <xsl:choose>
         ...
         <xsl:when test="$output='xhtml'">
            <html xmlns:gn="http://www.geonames.org/ontology#"
               ...
               <body>
                  <xsl:apply-templates select="/TEI:TEI/TEI:text/TEI:body" mode="xhtml"/>
               </body>
            </html>
         </xsl:when>
      </xsl:choose>
   ...
   <xsl:template match="TEI:head" mode="xhtml">
   ...
   <xsl:template match="TEI:pb" mode="xhtml">
