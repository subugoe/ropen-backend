...
declare function local:generate-cloud-xslt ($text as node()*, $facets as xs:string*) as node() {
    let $transform := doc(concat($archeao18conf:transformationsBase, 'cloud2.xsl'))
    let $params := <parameters>
                   <param name="baseuri" value="{$archeao18conf:transformationRestBase}"/>
               </parameters>
    return transform:stream-transform($text , $transform, $params)
};
...
let $cloud := local:generate-cloud-xslt($text, $facets)
